...
REM ============================================================
REM  UC GAA Maya 2026 - Fall Semester Lab Installer (SILENT / UNATTENDED)
REM
REM  Identical to Install_UC_TOOLS_FALL_2026.bat -- same steps, same
REM  robocopy calls, same admin-elevation requirement -- except this
REM  variant never blocks waiting for a keypress, so it's safe to wire
REM  into an automated Computer Startup Script or a Scheduled Task
REM  triggered "at log on of any user" (running as SYSTEM/elevated,
REM  since students logging into lab machines aren't admins and this
REM  still needs to write to C:\Cincy, lock pluginPrefs.mel, and set
REM  HKLM registry keys the same as before).
REM
REM  Use Install_UC_TOOLS_FALL_2026.bat instead for manual/by-hand runs
REM  where a human is present to read the final summary and press a key.
REM
...
REM  silently leaving a partial file or folder behind. A compact
REM  C:\Cincy\logs\install\install_summary.txt is also written on every
REM  run (overwritten, not appended) with just a pass/fail line and a
REM  timestamp, so automated monitoring can check the outcome of the
REM  last run without parsing the full robocopy log.
REM
...
set "SUMMARY_LOG=%LOG_DIR%\install_summary.txt"

echo.
echo ============================================================
echo  UC GAA Maya 2026 Lab Setup (silent)
echo ============================================================
echo.

REM -- 1. Create local folder structure -------------------------
echo [1/20] Creating local folder structure...
...
echo [1b/20] Cleaning up legacy folders...
...
echo [3/20] Deploying userSetup.mel...
...
echo [4/20] Deploying userPrefs.mel...
...
echo [5/20] Deploying pluginPrefs.mel...
...
echo [16/20] Deploying python_libs...
...
echo [19/20] Registering shottracker:// URI scheme...
...
REM -- 20. Deploy Shot Tracker desktop shortcut --------------------
...
REM     the way through.
echo [20/20] Deploying Shot Tracker desktop shortcut...
...
if "%HAD_ERRORS%"=="1" (
    ...
    (
        ...
        echo One or more steps failed to copy fully. See %INSTALL_LOG%
    ) > "%SUMMARY_LOG%"
) else (
