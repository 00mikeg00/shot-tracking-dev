...
Assignments-only. The capstone film pipeline (Layout -> Blocking ->
Animation -> Lighting) does NOT use this self-lock/GAA-Save flow -- each
step is its own top-level, coordinator-approved stage with a single
continuously versioned file (see CapstoneLayout.py/CapstoneBlocking.py/
CapstoneAnimation.py/CapstoneLighting.py), versioned via SaveNextVersion.py
instead. An earlier iteration of this file had a _do_save_capstone() path
for a Blocking/Blocking Plus/Polish self-lock sub-step model that has since
been dropped entirely, per explicit product direction.
...
and is shared with its own silent launcher-triggered run() and GAAOpen.py --
this file is a thin PySide6 UI layer over it, so save/open/launcher always
agree on step derivation and naming.
...
def _do_save(prompt_for_lock):
    """
    Assignments-only -- see module docstring for why the capstone film
    pipeline doesn't use this flow at all.
    """
    ...
    if not assignment:
        _error_dialog(
            ...
            "Click OPEN on an assignment in Shot Tracker first. (Film shots don't "
            "use GAA Save -- use SaveNextVersion.py to version those.)"
        )
        return
